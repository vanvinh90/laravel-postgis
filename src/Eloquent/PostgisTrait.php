...
trait PostgisTrait
{
  ...
  protected function geogFromText(GeometryInterface $geometry, $srid = 4326)
  {
    return $this->getConnection()->raw(sprintf(
      "%s.ST_GeogFromText('SRID=%d;%s')",
      function_exists('config') ? config('postgis.schema') : 'public',
      $srid,
      $geometry->toWKT()
    ));
    // return $this->getConnection()->raw(sprintf(
    //   "%s.ST_SetSRID( ST_MakePoint(%s, %s), %d )::geography",
    //   function_exists('config') ? config('postgis.schema') : 'public',
    //   $geometry->getLat(),
    //   $geometry->getLng(),
    //   $srid
  ...
  public function asWKT(GeometryInterface $geometry, $attrs)
  {
    switch (strtoupper($attrs['geomtype'])) {
      ...
      case 'GEOGRAPHY':
      default:
        return $this->geogFromText($geometry, $attrs['srid']);
        break;
    }
  }
